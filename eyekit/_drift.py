...
def cluster(fixation_XY, line_Y):
	try:
		...
		raise ImportError('The cluster method requires SciPy. Run "pip install scipy" to use this method.')
...
def regress(fixation_XY, line_Y, k_bounds=(-0.1, 0.1), o_bounds=(-50, 50), s_bounds=(1, 20)):
	try:
		...
		raise ImportError('The regress method requires SciPy. Run "pip install scipy" to use this method.')
	n = len(fixation_XY)
	m = len(line_Y)
...
def split(fixation_XY, line_Y):
	try:
		...
		raise ImportError('The split method requires SciPy. Run "pip install scipy" to use this method.')
	n = len(fixation_XY)
