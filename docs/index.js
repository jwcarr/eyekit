...
{
"ref":"eyekit.analysis.number_of_regressions",
"url":2,
"doc":"Given an interest area or collection of interest areas, return the number of regressions back to each interest area after the interest area was read for the first time. In other words, find the first fixation to exit the interest area and then count how many times the reader returns to the interest area from the right (or from the left in the case of right-to-left text). Returns a dictionary in which the keys are interest area IDs and the values are counts.",
"func":1
},
{
...
"url":5,
"doc":"Draw an arbitrary line on the image from  start_xy to  end_xy .  stroke_width is set in points for vector output or pixels for PNG output. If  dashed is  True , the line will have a dashed style (or a custom dash pattern can be supplied, e.g.  dashed=(1,2,4,2) ).  opacity controls the opacity of the line and should be set between 0 (fully transparent) and 1 (fully opaque).",
"func":1
},
{
"ref":"eyekit.vis.Image.draw_circle",
"url":5,
"doc":"Draw an arbitrary circle on the image centered at  x ,  y and with some  radius .  stroke_width is set in points for vector output or pixels for PNG output. If  dashed is  True , the line will have a dashed style (or a custom dash pattern can be supplied, e.g.  dashed=(1,2,4,2) ). If no  color or  fill_color is provided,  color will default to black.  opacity controls the opacity of the circle and should be set between 0 (fully transparent) and 1 (fully opaque).",
"func":1
},
{
"ref":"eyekit.vis.Image.draw_rectangle",
"url":5,
"doc":"Draw a rectangle on the image. You can pass in some box-like object, such as an  eyekit.text.InterestArea , or you can specify an x, y, width, and height to draw an arbitrary rectangle.  stroke_width is set in points for vector output or pixels for PNG output. If  dashed is  True , the line will have a dashed style (or a custom dash pattern can be supplied, e.g.  dashed=(1,2,4,2) ). If no  color or  fill_color is provided,  color will default to black.  opacity controls the opacity of the rectangle and should be set between 0 (fully transparent) and 1 (fully opaque).",
"func":1
},
{
"ref":"eyekit.vis.Image.draw_annotation",
"url":5,
"doc":"Draw arbitrary text on the image located at  x ,  y . If no font is set, the default font will be used (see  set_default_font ).  font_size is set in points for vector output or pixels for PNG output.  anchor controls the anchor point (alignment) of the annotation and may be set to  left ,  center , or  right .",
"func":1
},
{
...
"url":5,
"doc":"By default, each image caption is prefixed with a letter,  (A) ,  (B) ,  (C) , etc. If you want to turn this off, call  Figure.set_lettering(False) prior to saving. If no font is set, the default font will be used (see  set_default_font ).",
"func":1
},
{
